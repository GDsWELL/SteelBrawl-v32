namespace Supercell.Laser.Server.Message
{
    ...
    public class MessageManager
    {
        ...
        public void ShowLobbyInfo()
        {
            ...
            {
                LobbyData = $"BeastBrawl\nTG:@Beast_Brawls\n{abd}Online: {Sessions.Count}\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
                PlayersCount = 0
            };
            Connection.Send(b);
        }
        ...
        private void RequestDBChangesReceived(RequestDBChanges message)
        {
            ...
            else if (cmd[0] == "chat_unmute")
            {
                ...
                Notification wn = new Notification
                {
                    Id = 81,
                    //Time = DateTime.UtcNow,
                    MessageEntry = $"<c25c42b>Блокировка чата твоего аккаунта была снята досрочно</c>"
                };
            ...
            else if (cmd[0] == "chat_mute")
            {
                ...
                Notification wwn = new Notification
                {
                    Id = 81,
                    //Time = DateTime.UtcNow,
                    MessageEntry = $"Твоему аккаунт был временно отключён чат за нарушение правил.\n<ceb4034>Блокировка продлиться до {account.Home.ChatBanEndTime} UTC</c>\nПоследующие нарушения могут привести к блокировке чата навсегда"
                };
            ...
            else if (cmd[0] == "addbrawler" || cmd[0] == "addskin")
            {
                ...
                string d = sc ? LogicLongCodeGenerator.ToCode(long.Parse(cmd[1])) : cmd[1];
                Connection.Socket.Send(System.Text.Encoding.UTF8.GetBytes($"Done: added ксвкакое-то {int.Parse(cmd[2])} for {d}!"));
            }
                ...
                switch (cmd[0])
                {
                    case "status":
                        ...
                        response.Entry.Message = $"Server Status:\n" +
                            $"Server Game Version: v30\n" +
                            $"Server Build: v4 from 17.09.2024\n" +
                            $"Resources Sha: {Fingerprint.Sha}\n" +
                            $"Environment: Prod\n" +
                            $"Server Time: {DateTime.Now} EEST\n" +
                            $"Players Online: {Sessions.Count}\n" +
                            $"Memory Used: {megabytesUsed} MB\n" +
                            $"DEV: Well and Veist and FidBon\n"
                           +
                            $"Uptime: {formattedTime}\n"; 
                        Connection.Send(response);
                        break;
                    case "help":
                        response.Entry.Message = $"Avaliable commands:\n/help - show all server commands list\n/status - show server status\n\n--- BeastID ---\n\nРегестрация аккауна:\n/register [логин] [пароль] [повторение пароля] \n\nВосстановить аккаунт:\n/login [логин] [пароль]\n\nСменить логин аккаунта:\n/changelogin [новый логин] [новый логин]\n\nСменить пароль аккаунта:\n/changepassword [новый пароль] [новый пароль]"; // /usecode [code] - use bonus code
                        Connection.Send(response);
                        break;
                    case "debug":
                    ...
                    /*case "loadbycode":
                        ...
                        if (plraccount11.Code == cmd[2])
                        {
                            if (plraccount11.AccountId = playerId)
                            {
                                ...
                                {
                                    ErrorCode = 1,
                                    Message = "[MiokiID] Account loaded by tag! Tag: " + cmd[1]
                                });
                            }
                        }
                        break;
                    ...
                    case "mute":
                                ...
                                {
                                    Id = 81,
                                    MessageEntry = "Для твоего аккаунта временно отключены социальные функции\nЕсли ты думаешь что произошла ошибка, свяжись с администрацией"
                                };
                            ...
                            case "giveprem":
                                ...
                                Notification n = new Notification
                                {
                                    ...
                                    DonationCount = 30,
                                    //TimePassed = 
                                    MessageEntry = $"<c6>Vip статус активирован/продлён до {dsadasd.Home.PremiumEndTime} UTC, спасибо за поддержку сервера!</c>"
                                };
                                ...
                                // Отправляем сообщение об успешной выдаче VIP статуса в response.Entry.Message
                                response.Entry.Message = $"VIP успешно выдан игроку с тегом {cmd[1]}";
                                Connection.Send(response);
                                break;
                            case "gems":
                                if (HomeMode.Avatar.AccountId != 1)
                            ...
                            case "givevip":
                                ...
                                Notification n1 = new Notification
                                {
                                    ...
                                    DonationCount = 30,
                                    //TimePassed = 
                                    MessageEntry = $"<c6>Vip статус активирован/продлён до {dsadasd1.Home.PremiumEndTime} UTC, спасибо за поддержку сервера!</c>"
                                };
                    ...
                    case "addMega":
                        ...
                        break;
                    case "register":
                        string _message = "Неизвестная ошибка!";
                        bool login_not_used = true;
                        Console.WriteLine(_message);

                        if (cmd.Length != 4)
                        {
                            _message = "Использование команды: /register [логин] [пароль] [повторение пароля]";
                        ...
                        break;
                    case "login":
                        string _message1 = "Временно недоступно!";
                        int attempts = 0;
                        Console.WriteLine(_message1);

                        if (cmd.Length != 3)
                        {
                            _message1 = "Использование команды: /login [логин] [пароль]";
                        ...
                        if (true){
                            if (cmd[1] == HomeMode.Avatar.Login){
                                _message1 = "Нельзя зайти на аккаунт, на котором ты сейчас находишься!";
                                response.Entry.Message = _message1; // /usecode [code] - use bonus code
                                Connection.Send(response);
                            }
                            else{
                                ...
                                if (AccountLoad == null){
                                    _message1 = "Не удалось найти аккаунт! Проверьте правильность ввода логина или пароля!";
                                    response.Entry.Message = _message1; // /usecode [code] - use bonus code
                                    Connection.Send(response);
                                }
                                else{
                        ...
                        break;
                    case "changelogin":
                        string _message2 = "Неизвестная ошибка!";
                        Console.WriteLine(_message2);

                        if (cmd.Length != 3)
                        {
                            _message2 = "Использование команды: /changelogin [новый логин] [повторение нового логина]";
                        ...
                        if (cmd[1] == cmd[2]){
                            ...
                            _message2 = "Логины не совпадают!";
                            response.Entry.Message = _message2; // /usecode [code] - use bonus code
                            Connection.Send(response);
                            break;
                        }
                    case "changepassword":
                        string _message3 = "Неизвестная ошибка!";
                        Console.WriteLine(_message3);

                        if (cmd.Length != 3)
                        {
                            _message3 = "Использование команды: /changepassword [новый пароль] [повторение нового пароля]";
                    ...
                    case "resetseason":
                        ...
                        for (int accid = 1; accid <= lastAccId2; accid++)
                        {
                            ...
                            if (thisAcc.Avatar.Trophies >= 550)
                            {
                                ...
                                int[] StarPointsSeasonRewardAmount = { 100, 150, 200, 250, 260, 270, 300, 350, 400, 420, 440, 480, 400, 450, 460, 470, 480, 500 };
                                int[] StarPointsTrophiesInReset = { 525, 550, 600, 650, 700, 725, 750, 775, 800, 825, 850, 875, 900, 925, 950, 1000, 1100, 1225 };
            ...
            else if (HomeMode.Avatar.IsCommunityBanned)
            {
                AllianceStreamEntryMessage response = new()
                {
                    Entry = new AllianceStreamEntry
                    {
                        ...
                        Id = alliance.Stream.EntryIdCounter + 1,
                        AuthorRole = AllianceRole.Member,
                        Message = "Это сообщение не видят другие учасники, для общения в чате дождись конца блокривки!",
                        Type = 2
                    }
                };
                Connection.Send(response);
        ...
        private void AskForBattleEndReceived(AskForBattleEndMessage message)
        {
            ...
            if (DateTime.UtcNow > HomeMode.Home.PremiumEndTime && HomeMode.Avatar.PremiumLevel > 1)
            {
                ...
                HomeMode.Home.NotificationFactory.Add(new Notification
                {
                    Id = 81,
                    //TimePassed = 
                    MessageEntry = $"Привет!\n У тебя закончился твой vip статус\nпродлить его можно у одного из администраторов, тг: @well0ff"
                });
            }
                ...
                if (HomeMode.Avatar.PremiumLevel > 0)
                {
                    switch (HomeMode.Avatar.PremiumLevel)
                    {
                        case 1:
                            ...
                            {
                                underdogTrophiesResult += (int)Math.Floor((double)Trophies[0] / 2) * 2;
                                trophiesResult += (int)Math.Floor((double)Trophies[0] / 2) * 2;
                            }
                            else if (location.GameMode == "BattleRoyale" || location.GameMode == "BattleRoyaleTeam")
                            {
                                underdogTrophiesResult += (int)Math.Floor((double)Math.Abs(Trophies[message.Rank - 1]) / 2) * 2;
                                trophiesResult += (int)Math.Floor((double)Math.Abs(Trophies[message.Rank - 1]) / 2) * 2;
                            }
                            break;
        ...
        private void GetLeaderboardReceived(GetLeaderboardMessage message)
        {
            if (message.LeaderboardType == 1)
            {
                ...
                {
                    LeaderboardType = 1,
                    Region = message.IsRegional ? "RU" : null
                };
        ...
        private void LoginReceived(AuthenticationMessage message)
        {
            ...
            if (Sessions.IsSessionActive(account.Avatar.AccountIdRedirect))
            {
                    ...
                    session.GameListener.SendTCPMessage(new AuthenticationFailedMessage()
                    {
                        Message = "Другое устройство подключилось к это игре!"
                    });
            ...
            if (message.Major != 29)
            {
                ...
                loginFailed.Message = "Ты опять долбаеб и сделал что-то не так!";
                loginFailed.UpdateUrl = "https://t.me/";
                Connection.Send(loginFailed);
            ...
            if (account.Avatar.Banned)
            {
                ...
                if (account.Avatar.Banned){
                    ...
                    if (account.Avatar.BanTime[1] == 99){
                        ban_time = "\nАккаунт заблокирован навсегда без возможности разблокировки!";
                    }
                        ...
                        ban_time = "\nБлокировка действует до " + day_ban + "." + month_ban + "\nЗа повторные нарушения ваш аккаунт может быть заблокирован навсегда!";
                    }

                    if (account.Avatar.BannedReason == 1){
                        ban_message = "Ваш аккаунт заблокирован!\nПричина: недопустимый ник!";
                    }
                    else if (account.Avatar.BannedReason == 2){
                        ban_message = "Ваш аккаунт заблокирован!\nПричина: недопустимое поведение!";
                    }
                    else if (account.Avatar.BannedReason == 3){
                        ban_message = "Ваш аккаунт заблокирован!\nПричина: намеренное экспулатация ошибок!";
                    }
                    else if (account.Avatar.BannedReason == 4){
                        ban_message = "Ваш аккаунт заблокирован!\nПричина: пользование сторонним программным обеспечением!";
                    }
                    else if (account.Avatar.BannedReason == 5){
                        ban_message = "Ваш аккаунт заблокирован!\nПричина: винтрейдинг!";
                    }
                    else if (account.Avatar.BannedReason == 6){
                        ban_message = "Ваш аккаунт заблокирован!\nПричина: создание винтрейдинг группировок!";
                    }
                    else if (account.Avatar.BannedReason == 99){
                        ban_message = "Ваш аккаунт был перенесён на другое устройство!\nДля продолжения игры очистите данные приложения!";
                    }
                    else{
                        ban_message = "Ваш аккаунт заблокирован!";
                    }
            ...
            string GameVersion = "30.145";

            if (message.ClientVersion != GameVersion)
            {
                AuthenticationFailedMessage loginFailed = new AuthenticationFailedMessage();
                loginFailed.ErrorCode = 8;
                loginFailed.Message = "Вышло новое обновление BeastBrawl!\nДля продолжения игры быстрей скачивай обновление\nBeastBrawl " + message.ClientVersion + " => " + GameVersion;
                // loginFailed.Message = "ТЕХ.ПЕРЕРЫВ!!\n\nТЕСТИМ ОБНОВЛЕНИЕ....";
                loginFailed.UpdateUrl = "https://t.me/Beast_Brawls";
                Connection.Send(loginFailed);
                return;
            }
